...
async function main() {
  ...
  const kr = await prisma.keyResult.create({ data: { objectiveId: obj.id, ownerMembershipId: membership.id, title: 'Increase activation rate', metricUnit: '%', baseline: 28, target: 40 } });
